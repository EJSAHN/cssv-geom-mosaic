...
7) **(Add-on)** attach nearest-neighbor “context/provenance” labels using public-record descriptors.
...
## Add-ons: nearest-neighbor context / provenance annotation (post-hoc)

This repository provides two optional post-hoc add-ons:

- **Add-on A (within-dataset nearest-neighbor join):** uses the core distance matrix to assign each genome its closest *within* the analyzed set (addon_01 + addon_02).
- **Add-on B (reference-panel projection):** projects a query set (e.g., core48) onto a separate reference panel (e.g., PMPP87) using the same k-mer representation (addon_03).

### Add-on A: within-dataset nearest-neighbor join (addon_01 + addon_02)
...
### Add-on B: reference-panel projection (addon_03)

This add-on assigns each query genome its nearest neighbor **in a separate reference panel** using an alignment-free k-mer cosine distance.

Typical use:
- **Query set:** the 48 core genomes (`seq/core48.fasta`)
- **Reference panel:** PMPP86 plus an added NCBI reference (`seq/cibv.fa`, 87 sequences)

#### Inputs
- Query FASTA (multi-record): e.g., `seq/core48.fasta`
- Reference FASTA (multi-record): e.g., `seq/cibv.fa`
- k-mer length: `--k 4` (match the core pipeline’s k when you want direct comparability)

#### Outputs
- `projection_nearest_reference.tsv`  
  Per-query nearest reference accession, distance, and reference header/provenance text.
- `projection_token_summary.tsv`  
  Frequency summary of recurring isolate-like / regional descriptors among projected nearest neighbors.
...
```bash
python addons/addon_03_reference_panel_projection.py --query_fasta seq/core48.fasta --ref_fasta seq/cibv.fa --k 4
```

> Notes:
> - If query and reference share accessions, the script excludes self-hits when IDs match exactly.
> - Keep FASTA headers intact because isolate/regional descriptors are extracted from public record descriptions.
