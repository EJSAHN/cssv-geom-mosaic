#!/usr/bin/env python3
"""
Compare entropy statistics (mean/variance) between Top-N highest-scoring genomes and others.

Inputs:
- entropy_profiles.csv from cssv_entropy_analysis.py
  required columns: genome, start, entropy
- mosaic_ranked_genomes.csv from cssv_tree_mosaic_agreement.py
...
Usage (Windows):
python scripts/cssv_entropy_mosaic_ranked_test.py ^
  --entropy_profiles "results/entropy/entropy_profiles.csv" ^
  --ranked_genomes "gb_results/tree_mosaic_agreement/mosaic_ranked_genomes.csv" ^
  --out_dir "results/entropy/mosaic_ranked_entropy_test" ^
  --top_n 10 ^
  --perm 10000 ^
  --seed 0
"""
...
def save_boxplot(
    ...
    ax.boxplot([data_other, data_top], labels=["Others", "TopN highest-scoring"], showfliers=True)
...
def main() -> None:
    ...
    ap.add_argument("--ranked_genomes", dest="ranked_genomes", required=True, help="ranked-genome CSV with a genome column")
    ...
    ranked = pd.read_csv(args.ranked_genomes)
    ...
    if "genome" not in ranked.columns:
        raise ValueError("ranked-genome table is missing column: genome")

    # Normalize genome names to accessions
    ent["genome"] = ent["genome"].astype(str).map(tip_to_accession)
    ranked["genome"] = ranked["genome"].astype(str).map(tip_to_accession)

    topN = ranked.head(int(args.top_n))["genome"].tolist()
    top_set = set(topN)
    ...
    gstats["group"] = np.where(gstats["genome"].isin(top_set), "highest_score_topN", "other")
    ...
    # Split arrays
    top_mean = gstats.loc[gstats["group"] == "highest_score_topN", "mean_entropy"].to_numpy(dtype=float)
    oth_mean = gstats.loc[gstats["group"] == "other", "mean_entropy"].to_numpy(dtype=float)
    top_var = gstats.loc[gstats["group"] == "highest_score_topN", "var_entropy"].to_numpy(dtype=float)
    ...
    if not args.no_plots:
        save_boxplot(
            ...
            title=f"Top {args.top_n} highest-scoring vs Others: mean entropy",
            note_lines=[
        ...
        save_boxplot(
            ...
            title=f"Top {args.top_n} highest-scoring vs Others: entropy variance",
            note_lines=[
