...
def read_scores(path: Path) -> pd.DataFrame:
    ...
    The score column is expected to be ``mosaic_discordance_score``, matching
    the terminology used in the manuscript and public outputs.
    """
    df = pd.read_csv(path)
    if "genome" not in df.columns or "mosaic_discordance_score" not in df.columns:
        raise ValueError(
            f"Expected 'genome' and 'mosaic_discordance_score' columns in {path}. "
            f"Available columns: {list(df.columns)}"
        )
    df["mosaic_discordance_score"] = pd.to_numeric(df["mosaic_discordance_score"], errors="coerce")
    return df.sort_values("mosaic_discordance_score", ascending=False).reset_index(drop=True)
...
def run_agreement(repo: Path, agree_script: Path, orf_dist: Path, window_assignments: Path, agree_dir: Path,
                  orf_K: int, top_n: int, orf_name_split: str, dry_run: bool):
...
def main() -> None:
    ...
    for k, window, step, mosaic_K in mosaic_configs(args):
        ...
        run_agreement(repo, agree_script, orf_dist, gb_dir / "window_assignments.csv", agree_dir, orf_K, args.top_n, args.orf_name_split, args.dry_run)
        if args.dry_run:
            continue
        scores = read_scores(agree_dir / "mosaic_orf_merged_per_genome.csv")
        scores["rank"] = np.arange(1, len(scores) + 1)
        score_tables[name] = scores[["genome", "rank", "mosaic_discordance_score"]].copy()
        ...
        rows.append({
            ...
            "score_rank1": float(scores.iloc[0]["mosaic_discordance_score"]),
            "score_rankN": float(scores.iloc[min(args.top_n, len(scores))-1]["mosaic_discordance_score"]),
        })
        for _, r in scores.head(args.top_n).iterrows():
            top_rows.append({"analysis_type": "mosaic_parameter_sensitivity_orfK_fixed", "config": name, "rank": int(r["rank"]), "genome": r["genome"], "mosaic_discordance_score": float(r["mosaic_discordance_score"])})
    ...
    for orf_K in csv_int_list(args.orf_cluster_values):
        ...
        run_agreement(repo, agree_script, orf_dist, baseline_gb_dir / "window_assignments.csv", agree_dir,
                      orf_K, args.top_n, args.orf_name_split, args.dry_run)
        if args.dry_run:
            continue
        scores = read_scores(agree_dir / "mosaic_orf_merged_per_genome.csv")
        scores["rank"] = np.arange(1, len(scores) + 1)
        ...
        for _, r in scores.head(args.top_n).iterrows():
            top_rows.append({"analysis_type": "orf_cluster_K_sensitivity_baseline_mosaic_fixed", "config": name, "rank": int(r["rank"]), "genome": r["genome"], "mosaic_discordance_score": float(r["mosaic_discordance_score"])})
    ...
    baseline_scores = baseline.set_index("genome")["mosaic_discordance_score"]
    ...
    for name, tab in score_tables.items():
        ...
        if spearmanr is not None and len(common) >= 3:
            rho = float(spearmanr(baseline_scores.loc[common], tab.set_index("genome").loc[common, "mosaic_discordance_score"]).correlation)
        ov.append({
            "config": name,
