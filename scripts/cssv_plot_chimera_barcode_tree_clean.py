"""
Cleaner figure:
- Left: mosaic barcodes for top N chimera candidates
...
Usage:
python scripts/cssv_plot_chimera_barcode_tree_clean.py ^
  ...
  --out_prefix "results/tree_mosaic_agreement/chimera_top10_barcode_tree_clean" ^
  --top_n 10
"""
...
def main():
    ...
    ap.add_argument("--chimera", required=True)
    ...
    chim = pd.read_csv(args.chimera).copy()
    if "genome" not in chim.columns:
        raise ValueError("chimera_candidates.csv must have column 'genome'")
    ...
    draw_barcode(ax1, win, top, title=f"Top {len(top)} chimera candidates (barcode)")

    ax2 = fig.add_subplot(gs[0, 1])
    draw_tree_clean(ax2, Path(args.tree_newick), highlight, title="ORF3 NJ tree (chimera tips in red, *)")
