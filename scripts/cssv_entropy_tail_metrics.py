#!/usr/bin/env python3
"""
Tail/peak entropy metrics for Top-N mosaic-ranked genomes vs Others.
...
- mosaic_candidates.csv (column: genome)
...
def boxplot_two_groups(
    ...
    ax.boxplot([others, top], labels=["Others", "TopN mosaic-ranked"], showfliers=True)
...
def main() -> None:
    ...
    if "genome" not in chim.columns:
        raise ValueError("candidate ranking table missing column: genome")
    ...
    g["group"] = np.where(g["genome"].isin(top_set), "mosaic_topN", "other")
    ...
    for idx, (mcol, label) in enumerate(metrics_to_test):
        top = g.loc[g["group"] == "mosaic_topN", mcol].to_numpy(dtype=float)
        ...
        boxplot_two_groups(
            ...
            top=top,
            ylabel=label,
            title=f"Top mosaic-ranked{args.top_n} vs Others: {label}",
            note_lines=note,
            dpi=args.dpi,
        )
