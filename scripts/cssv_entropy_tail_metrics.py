#!/usr/bin/env python3
"""
Tail/peak entropy metrics for Top-N highest-scoring genomes vs Others.
...
- mosaic_ranked_genomes.csv (column: genome)
...
python scripts/cssv_entropy_tail_metrics.py ^
  ...
  --out_dir "results/entropy/mosaic_ranked_entropy_tail_test" ^
  --top_n 10 ^
  --perm 10000 ^
  --seed 0
"""
...
def boxplot_two_groups(
    ...
    ax.boxplot([others, top], labels=["Others", "TopN highest-scoring"], showfliers=True)
...
def main() -> None:
    ...
    if "genome" not in ranked.columns:
        raise ValueError("ranked-genome table missing column: genome")
    ...
    top_list = ranked.head(int(args.top_n))["genome"].tolist()
    top_set = set(top_list)

    g = per_genome_tail_metrics(ent)
    g["group"] = np.where(g["genome"].isin(top_set), "highest_score_topN", "other")
    ...
    for idx, (mcol, label) in enumerate(metrics_to_test):
        top = g.loc[g["group"] == "highest_score_topN", mcol].to_numpy(dtype=float)
        ...
        if not args.no_plots:
            boxplot_two_groups(
                ...
                top=top,
                ylabel=label,
                title=f"Top {args.top_n} highest-scoring vs Others: {label}",
                note_lines=note,
                dpi=args.dpi,
            )
