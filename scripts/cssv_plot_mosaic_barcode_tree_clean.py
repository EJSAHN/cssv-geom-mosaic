"""
Cleaner figure:
- Left: mosaic barcodes for top N mosaic/recombinant-like candidates
- Right: ORF3 NJ tree, tips renamed to accession only
        candidates highlighted (red + trailing '*')

Outputs vector PDF + PNG 300dpi.

Usage:
python scripts/cssv_plot_mosaic_barcode_tree_clean.py ^
  --candidates "results/tree_mosaic_agreement/mosaic_candidates.csv" ^
...
def main():
    ...
    ap.add_argument("--candidates", "--mosaic", dest="candidates", required=True, help="candidate ranking CSV")
    ...
    chim = pd.read_csv(args.candidates).copy()
    if "genome" not in chim.columns:
        raise ValueError("candidate ranking CSV must have column 'genome'")
    chim["genome"] = chim["genome"].astype(str).str.strip()
    top = chim.head(int(args.top_n))["genome"].tolist()
    highlight = set(top)
    ...
    draw_barcode(ax1, win, top, title=f"Top {len(top)} mosaic/recombinant-like candidates (barcode)")

    ax2 = fig.add_subplot(gs[0, 1])
    draw_tree_clean(ax2, Path(args.tree_newick), highlight, title="ORF3 NJ tree (mosaic-ranked tips in red, *)")
