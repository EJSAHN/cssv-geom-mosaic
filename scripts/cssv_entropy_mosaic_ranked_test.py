#!/usr/bin/env python3
"""
Compare entropy statistics (mean/variance) between Top-N mosaic-ranked genomes and others.

Inputs:
- entropy_profiles.csv from cssv_entropy_analysis.py
  required columns: genome, start, entropy
- mosaic_candidates.csv from cssv_tree_mosaic_agreement.py
...
python scripts/cssv_entropy_mosaic_ranked_test.py ^
  ...
  --candidates "gb_results/tree_mosaic_agreement/mosaic_candidates.csv" ^
...
def save_boxplot(
    ...
    ax.boxplot([data_other, data_top], labels=["Others", "TopN mosaic-ranked"], showfliers=True)
...
def main() -> None:
    ...
    ap.add_argument("--candidates", "--mosaic_candidates", dest="candidates", required=True, help="candidate ranking CSV with a genome column")
    ...
    chim = pd.read_csv(args.candidates)
    ...
    if "genome" not in chim.columns:
        raise ValueError("candidate ranking table is missing column: genome")
    ...
    gstats["group"] = np.where(gstats["genome"].isin(top_set), "mosaic_topN", "other")
    ...
    # Split arrays
    top_mean = gstats.loc[gstats["group"] == "mosaic_topN", "mean_entropy"].to_numpy(dtype=float)
    oth_mean = gstats.loc[gstats["group"] == "other", "mean_entropy"].to_numpy(dtype=float)
    top_var = gstats.loc[gstats["group"] == "mosaic_topN", "var_entropy"].to_numpy(dtype=float)
    ...
    save_boxplot(
        ...
        title=f"Top mosaic-ranked{args.top_n} vs Others: mean entropy",
        note_lines=[
    ...
    save_boxplot(
        ...
        title=f"Top mosaic-ranked{args.top_n} vs Others: entropy variance",
        note_lines=[
