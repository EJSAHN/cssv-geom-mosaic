...
def make_supplementary_s1_excel(
    ...
    Includes:
    - Core pipeline outputs (genome summary/embedding, distance matrices, mosaic/orf, chimera)
    - Optional switchpoint_postprocess outputs
    - NEW: Entropy module outputs (entropy profiles, JSD distance, entropy/chimera tests, JSD concordance)
    ...
    for p in sorted(tables_dir.glob("Table1_chimera_top*.xlsx")):
        table1_xlsx = p
        break
    ...
    # Chimera entropy tests
    entropy_mean_var_tests = try_csv(entropy_root / "chimera_entropy_test" / "permutation_tests_summary.csv")
    entropy_tail_tests = try_csv(entropy_root / "chimera_entropy_tail_test" / "tail_permutation_tests_summary.csv")
    per_genome_entropy_tail_metrics = try_csv(entropy_root / "chimera_entropy_tail_test" / "per_genome_entropy_tail_metrics.csv")
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        ...
        add_sheet(writer, "entropy_mean_var_tests", entropy_mean_var_tests, entropy_root / "chimera_entropy_test" / "permutation_tests_summary.csv")
        add_sheet(writer, "entropy_tail_tests", entropy_tail_tests, entropy_root / "chimera_entropy_tail_test" / "tail_permutation_tests_summary.csv")
        add_sheet(writer, "per_genome_entropy_tail", per_genome_entropy_tail_metrics, entropy_root / "chimera_entropy_tail_test" / "per_genome_entropy_tail_metrics.csv")
...
def main() -> None:
    ...
    ap.add_argument("--top_n", type=int, default=10, help="Top N chimera candidates for Fig6 and Table1")
