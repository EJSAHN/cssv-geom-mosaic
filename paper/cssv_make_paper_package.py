...
- <out_dir>/tables/Table1_mosaic_ranked_topN.xlsx (+ .csv)
...
def _draw_barcode(ax: plt.Axes, win: pd.DataFrame, genomes: List[str]) -> None:
    ...
    ax.set_title("Mosaic barcodes (top mosaic-ranked candidates)")
...
def make_figure6(
    ...
    """
    chim = pd.read_csv(candidate_csv)
    if "genome" not in chim.columns:
        raise ValueError("candidate ranking CSV must have 'genome' column")
...
def make_table1_excel(
    ...
    """
    Create Table 1 (top N mosaic-ranked candidates) as:
      - Excel .xlsx
      - CSV
    """
    chim = pd.read_csv(candidate_csv)
    ...
    if "genome" not in chim.columns or "genome" not in merged.columns:
        raise ValueError("candidate ranking CSV and merged.csv must both have 'genome' column")
    ...
    _ensure_dir(out_dir)
    xlsx_path = out_dir / f"Table1_mosaic_ranked_top{top_n}.xlsx"
    csv_path = out_dir / f"Table1_mosaic_ranked_top{top_n}.csv"
...
def make_supplementary_s1_excel(
    ...
    Includes:
    - Core pipeline outputs (genome summary/embedding, distance matrices, mosaic/orf, mosaic-discordance ranking)
    - Optional switchpoint_postprocess outputs
    - NEW: Entropy module outputs (entropy profiles, JSD distance, entropy tests for mosaic-ranked candidates, JSD concordance)
    ...
    candidate_path = agreement_dir / "mosaic_ranked_candidates.csv"
    mosaic_ranked_candidates = try_csv(candidate_path)
    ...
    for p in sorted(tables_dir.glob("Table1_mosaic_ranked_top*.xlsx")):
        table1_xlsx = p
        break
    ...
    # Entropy tests for mosaic-ranked candidates
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        ...
        add_sheet(writer, "mosaic_ranked_candidates", mosaic_ranked_candidates, candidate_path)
...
def main() -> None:
    ...
    ap.add_argument("--top_n", type=int, default=10, help="Top N mosaic/recombinant-like candidates for Fig6 and Table1")
    ...
    if not args.skip_figures:
        ...
        make_figure6(
            candidate_csv=agreement_dir / "mosaic_ranked_candidates.csv",
