...
- <out_dir>/tables/Table1_mosaic_complexity_topN.xlsx (+ .csv)
...
def _draw_barcode(ax: plt.Axes, win: pd.DataFrame, genomes: List[str]) -> None:
    ...
    ax.set_title("Mosaic barcodes (ten highest-scoring genomes)")
...
def _draw_tree_highlight(ax: plt.Axes, newick: Path, highlight: set[str]) -> None:
    ...
    ax.set_title("ORF3 NJ tree (highest-scoring genomes in red, *)")
    ax.set_axis_off()
    for txt in ax.texts:
...
def make_figure6(
    ...
    if "genome" not in ranked.columns:
        raise ValueError("ranked-genome CSV must have 'genome' column")
    ranked["genome"] = ranked["genome"].astype(str).map(_tip_to_accession)
    top = ranked.head(int(top_n))["genome"].tolist()
    highlight = set(top)
    ...
    fig.tight_layout()
    out_pdf = out_dir / "Figure6_mosaic_barcode_tree.pdf"
    out_png = out_dir / "Figure6_mosaic_barcode_tree.png"
    _save_fig(fig, out_pdf, out_png, dpi=dpi)
    return out_pdf, out_png
...
def make_table1_excel(
    ...
    """
    Create Table 1 (top N highest-scoring genomes) as:
      - Excel .xlsx
      - CSV
    """
    ranked = pd.read_csv(ranked_csv)
    merged = pd.read_csv(merged_csv)
    if "genome" not in ranked.columns or "genome" not in merged.columns:
        raise ValueError("ranked-genome CSV and merged.csv must both have 'genome' column")
    ...
    _ensure_dir(out_dir)
    xlsx_path = out_dir / f"Table1_mosaic_complexity_top{top_n}.xlsx"
    csv_path = out_dir / f"Table1_mosaic_complexity_top{top_n}.csv"
...
def make_supplementary_s1_excel(
    ...
    Includes:
    - Core pipeline outputs (genome summary/embedding, distance matrices, mosaic/orf, mosaic-complexity ranking)
    - Optional switchpoint_postprocess outputs
    - NEW: Entropy module outputs (entropy profiles, JSD distance, entropy tests for highest-scoring genomes, JSD concordance)

    Entropy module auto-discovery:
    - Looks under: <gb_dir.parent>/results/entropy/
      (e.g., C:\\projects\\CSSV\\gb_results -> C:\\projects\\CSSV\\results\\entropy)
    - If not found, marks sheets as MISSING in manifest and continues.
    """
    ...
    def add_sheet(writer, sheet_name: str, df: Optional[pd.DataFrame], src: Path):
        ...
        if isinstance(df, pd.DataFrame) and df.shape[0] == 0:
            manifest_rows.append({"sheet": sheet_name, "status": "EMPTY", "source": str(src)})
            # still write header if possible
    ...
    def try_csv(path: Path, index_col0: bool = False) -> Optional[pd.DataFrame]:
    ...
    def try_square_to_long_df(path: Path) -> Optional[pd.DataFrame]:
        """
        Read square matrix CSV (index_col=0) and return a rectangular dataframe with
        an explicit 'name' column for Excel friendliness.
        """
        df = try_csv(path, index_col0=True)
        if df is None:
            return None
    ...
    # k4 distance matrix (could be exactly k4_cosine_distance.csv)
    k4_cos = None
    if (gb_dir / "k4_cosine_distance.csv").exists():
        k4_cos = try_square_to_long_df(gb_dir / "k4_cosine_distance.csv")
    else:
        # fallback: any k*_cosine_distance.csv
        for p in gb_dir.glob("k*_cosine_distance.csv"):
            k4_cos = try_square_to_long_df(p)
            break
    ...
    orf3_dist = try_square_to_long_df(orf3_phylogeny_dir / "pairwise_identity_distance.csv")
    ...
    ranked_path = agreement_dir / "mosaic_ranked_genomes.csv"
    mosaic_ranked_genomes = try_csv(ranked_path)
    mosaic_orf_merged = try_csv(agreement_dir / "mosaic_orf_merged_per_genome.csv")

    # Optional switchpoint postprocess
    ...
    table1_xlsx = None
    table1_df = None
    # load the first matching Table1 excel from tables_dir
    for p in sorted(tables_dir.glob("Table1_mosaic_complexity_top*.xlsx")):
        table1_xlsx = p
        break
    if table1_xlsx and table1_xlsx.exists():
        try:
    ...
    # -----------------------------
    # NEW: Entropy module discovery
    # -----------------------------
    entropy_root = gb_dir.parent / "results" / "entropy"
    entropy_profiles = try_csv(entropy_root / "entropy_profiles.csv")
    jsd_matrix = try_square_to_long_df(entropy_root / "jsd_distance_matrix.csv")

    # JSD concordance summaries
    jsd_vs_cosine_summary = try_csv(entropy_root / "compare_jsd_vs_cosine" / "compare_distances.summary.csv")
    jsd_vs_orf3_summary = try_csv(entropy_root / "compare_jsd_vs_orf3" / "compare_distances.summary.csv")

    # Entropy tests for highest-scoring genomes
    entropy_mean_var_path = entropy_root / "mosaic_ranked_entropy_test" / "permutation_tests_summary.csv"
    entropy_mean_var_tests = try_csv(entropy_mean_var_path)
    entropy_tail_path = entropy_root / "mosaic_ranked_entropy_tail_test" / "tail_permutation_tests_summary.csv"
    entropy_tail_tests = try_csv(entropy_tail_path)
    per_genome_entropy_tail_path = entropy_root / "mosaic_ranked_entropy_tail_test" / "per_genome_entropy_tail_metrics.csv"
    per_genome_entropy_tail_metrics = try_csv(per_genome_entropy_tail_path)

    # Optionally: include genomes_used lists as sheets if present
    jsd_vs_cosine_genomes_used = None
    gpath = entropy_root / "compare_jsd_vs_cosine" / "compare_distances.genomes_used.txt"
    if gpath.exists() and gpath.stat().st_size > 0:
        lines = [ln.strip() for ln in gpath.read_text(encoding="utf-8").splitlines() if ln.strip()]
        jsd_vs_cosine_genomes_used = pd.DataFrame({"genome": lines})

    jsd_vs_orf3_genomes_used = None
    gpath2 = entropy_root / "compare_jsd_vs_orf3" / "compare_distances.genomes_used.txt"
    if gpath2.exists() and gpath2.stat().st_size > 0:
        lines = [ln.strip() for ln in gpath2.read_text(encoding="utf-8").splitlines() if ln.strip()]
        jsd_vs_orf3_genomes_used = pd.DataFrame({"genome": lines})
    ...
    with pd.ExcelWriter(out_xlsx, engine="openpyxl") as writer:
        ...
        add_sheet(writer, "switchpoint_orf_dist", sp_orf_dist, mosaic_orf_dir / "switchpoint_orf_distances.csv")
        ...
        add_sheet(writer, "compare_distances", compare_summary, compare_dir / "compare_distances.summary.csv")

        add_sheet(writer, "agreement_metrics", agreement_metrics, agreement_dir / "agreement_metrics.csv")
        add_sheet(writer, "contingency_orf_mosaic", contingency, agreement_dir / "contingency_orf_vs_mosaic.csv")
        add_sheet(writer, "mosaic_ranked_genomes", mosaic_ranked_genomes, ranked_path)
        ...
        add_sheet(writer, "switch_trans_matrix", sp_trans_mat, (switchpoint_post_dir / "switchpoint_transition_matrix.csv") if switchpoint_post_dir else Path("N/A"))
        add_sheet(writer, "switch_pos_density", sp_pos_dens, (switchpoint_post_dir / "switchpoint_position_density.csv") if switchpoint_post_dir else Path("N/A"))

        add_sheet(writer, "Table1", table1_df, table1_xlsx if table1_xlsx else Path("N/A"))

        # -----------------------------
        # NEW: Entropy module sheets
        # -----------------------------
        add_sheet(writer, "entropy_profiles", entropy_profiles, entropy_root / "entropy_profiles.csv")
        add_sheet(writer, "jsd_distance_matrix", jsd_matrix, entropy_root / "jsd_distance_matrix.csv")
        add_sheet(writer, "jsd_vs_cosine_summary", jsd_vs_cosine_summary, entropy_root / "compare_jsd_vs_cosine" / "compare_distances.summary.csv")
        add_sheet(writer, "jsd_vs_orf3_summary", jsd_vs_orf3_summary, entropy_root / "compare_jsd_vs_orf3" / "compare_distances.summary.csv")

        add_sheet(writer, "entropy_mean_var_tests", entropy_mean_var_tests, entropy_mean_var_path)
        add_sheet(writer, "entropy_tail_tests", entropy_tail_tests, entropy_tail_path)
        add_sheet(writer, "per_genome_entropy_tail", per_genome_entropy_tail_metrics, per_genome_entropy_tail_path)

        add_sheet(writer, "jsd_vs_cosine_genomes", jsd_vs_cosine_genomes_used, entropy_root / "compare_jsd_vs_cosine" / "compare_distances.genomes_used.txt")
        add_sheet(writer, "jsd_vs_orf3_genomes", jsd_vs_orf3_genomes_used, entropy_root / "compare_jsd_vs_orf3" / "compare_distances.genomes_used.txt")
...
# --------------------------
def main() -> None:
    ap = argparse.ArgumentParser(description="Build paper-ready figures, Table 1, and Supplementary Data S1 (multi-sheet Excel).")

    ap.add_argument("--gb_dir", required=True, help="Directory containing gb pipeline outputs (e.g., gb_results)")
    ap.add_argument("--mosaic_orf_dir", required=True, help="Directory containing mosaic_orf outputs (e.g., gb_results\\mosaic_orf)")
    ap.add_argument("--orf3_dir", required=True, help="Directory containing ORF3 proxy outputs (e.g., gb_results\\mosaic_orf\\longest_orfs)")
    ap.add_argument("--orf3_phylogeny_dir", required=True, help="Directory containing ORF3 phylogeny outputs (e.g., ...\\phylogeny)")
    ap.add_argument("--compare_dir", required=True, help="Directory containing distance comparison outputs (e.g., gb_results\\distance_compare)")
    ap.add_argument("--agreement_dir", required=True, help="Directory containing tree/mosaic agreement outputs (e.g., gb_results\\tree_mosaic_agreement)")
    ap.add_argument("--switchpoint_post_dir", default=None, help="Optional directory for switchpoint postprocess outputs (e.g., gb_results\\switchpoint_post)")

    ap.add_argument("--out_dir", required=True, help="Output root directory (will create figures/, tables/, supplementary/ under this)")
    ap.add_argument("--top_n", type=int, default=10, help="Top N highest-scoring genomes for Fig6 and Table1")
    ap.add_argument("--panel_case", choices=["upper", "lower"], default="upper", help="Panel label case")
    ap.add_argument("--dpi", type=int, default=300, help="PNG DPI")

    ap.add_argument("--skip_figures", action="store_true", help="Skip Figure 1–6 generation")
    ap.add_argument("--skip_table", action="store_true", help="Skip Table 1 generation")
    ap.add_argument("--skip_s1", action="store_true", help="Skip Supplementary Data S1 Excel generation")
    ...
    print(f"[RUN] cssv_make_paper_package -> {out_root}")

    if not args.skip_figures:
        print("[STEP] Generating Figures 1–6 ...")
        make_figure1(
        ...
        make_figure6(
            ranked_csv=agreement_dir / "mosaic_ranked_genomes.csv",
    ...
    if not args.skip_s1:
        print("[STEP] Generating Supplementary Data S1 Excel ...")
        ...
        print(f"[OK] Supplementary Data S1: {out_xlsx}")

    print("[SUCCESS] Paper package complete.")
