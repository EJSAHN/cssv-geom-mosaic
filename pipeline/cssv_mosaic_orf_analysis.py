...
def extract_switchpoints(
    ...
    """
    windows_df columns: [name,start,end,label]
    returns switchpoints with columns:
      [name, pos0, prev_label, new_label, prev_run_windows, new_run_windows]
    """
    need = {"name", "start", "end", "label"}
    if not need.issubset(set(windows_df.columns)):
        raise ValueError(f"window_assignments must have columns {sorted(need)}. Got {windows_df.columns.tolist()}")

    out_rows = []
    for name, g in windows_df.groupby("name", sort=False):
        gg = g.sort_values("start").reset_index(drop=True)
        labels = gg["label"].tolist()
        starts = gg["start"].astype(int).tolist()
        ends = gg["end"].astype(int).tolist()
        n = len(gg)
        if n <= 1:
            continue
        ...
        for i in range(1, n):
            if labels[i] != labels[i - 1]:
                runs.append((run_start, i - 1, labels[i - 1]))
                run_start = i
        runs.append((run_start, n - 1, labels[-1]))

        # create switchpoints between runs; filter by min_run on both sides
        for r in range(len(runs) - 1):
            ...
            if switchpoint_mode == "start":
                ...
                # midpoint between last window of prev run and first window of new run
                pos0 = int((ends[a1] + starts[b0]) // 2)
            else:
...
def main():
    ...
    if args.plot_top and args.plot_top > 0:
        # top genomes by number of switchpoints
