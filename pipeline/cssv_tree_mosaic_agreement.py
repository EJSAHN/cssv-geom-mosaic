"""
Tree/ORF distance vs mosaic-label agreement and mosaic/recombinant-like candidate ranking.
...
5) Ranks mosaic/recombinant-like candidates for follow-up (low purity and/or strong ORF-vs-mosaic disagreement).
...
def main():
    ...
    # Composite mosaic-discordance score used in the manuscript.
    merged["mosaic_discordance_score"] = (
    ...
    ranked = merged.sort_values(["mosaic_discordance_score", "mismatch", "dominant_frac"], ascending=[False, False, True])
    ranked.to_csv(out_dir / "mosaic_orf_merged_per_genome.csv", index=False)
    ranked.head(int(args.top_n)).to_csv(out_dir / "mosaic_ranked_candidates.csv", index=False)
    ...
    print(f"- Top candidates: {out_dir / 'mosaic_ranked_candidates.csv'}")
